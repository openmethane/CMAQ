...
!-------------------------------------------------------------------------------
  ...
  IF ( ( first ) .AND. ( mcip_now > met_startdate )  &
    ...
    ELSE
      WRITE (*,f9400) TRIM(pname), 'RAINC', rcode
      CALL graceful_stop (pname)
    ENDIF
    ...
    ELSE
      WRITE (*,f9400) TRIM(pname), 'RAINNC', rcode
      CALL graceful_stop (pname)
    ENDIF
    ...
    IF ( met_rain_bucket > 0.0 ) THEN  ! tipping bucket is on
      ...
      ELSE
        WRITE (*,f9400) TRIM(pname), 'I_RAINC', rcode
        CALL graceful_stop (pname)
      ENDIF
      ...
      ELSE
        WRITE (*,f9400) TRIM(pname), 'I_RAINNC', rcode
        CALL graceful_stop (pname)
      ENDIF
...
!-------------------------------------------------------------------------------
  ...
  IF ( rcode == nf90_noerr ) THEN
    ...
    qfx(nx,:) = qfx(nxm,:)
    qfx(:,ny) = qfx(:,nym)
    WRITE (*,f6000) 'LH       ', qfx(lprt_metx, lprt_mety), 'W m^-2'
  ELSE
